 ...
 * PRACTICE 6 - LOCALIZATION BY PARTICLE FILTERS
 *
 * Instructions:
...
#define NOMBRE "Villanueva_Aragon_Gabriel"
...
geometry_msgs::PoseArray get_initial_distribution(int N, float min_x, float max_x, float min_y, float max_y, float min_a, float max_a)
{
     ...
     * For the Euler angles ( for(size_t i=0; i<particles.poses.size(); i++)
        simulated_scans[i] = *occupancy_grid_utils::simulateRangeScan(map, particles.poses[i], real_sensor_info);
    return simulated_scans;roll, pitch, yaw) = (0,0,theta) the corresponding quaternion is
